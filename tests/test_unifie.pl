...
test1() :-
    ...
    unifie([f(_X,Y) ?= f(g(Z),h(a)), Z ?= f(Y)]),
    echoNL("true : OK"), nl.


test2() :-
    echoNL("[f(X,Y) ?= f(g(Z),h(a)), Z ?= f(X)]"),
    not(unifie([f(X,_Y) ?= f(g(Z),h(a)), Z ?= f(X)])),
    echoNL("false : OK"), nl.


test3() :-
    echoNL("[Z ?= f(Y), f(X,Y) ?= f(g(Z),h(a))]"),
    unifie([Z ?= f(Y), f(_X,Y) ?= f(g(Z),h(a))]),
    echoNL("true : OK"), nl.


test4() :-
    echoNL("[Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), b ?= W, f(X) ?= f(X)]"),
    unifie([Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), b ?= W, f(X) ?= f(X)]),
    echoNL("true : OK"), nl.


test5() :-
    echoNL("[Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), b ?= W, f(X) ?= f(h(X))]"),
    not(unifie([Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), b ?= W, f(X) ?= f(h(X))])),
    echoNL("false : OK"), nl.


test6() :-
    echoNL("[Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), f(A,B) = g(X,Y)]"),
    not(unifie([Z ?= f(Y), f(X,Y) ?= f(g(Z),h(_W)), f(_A,_B) = g(X,Y)])),
    echoNL("false : OK"), nl.


test7() :-
    echoNL("[Z ?= f(Y), f(X,Y) ?= f(g(Z),h(W)), b ?= a]"),
    not(unifie([Z ?= f(Y), f(_X,Y) ?= f(g(Z),h(_W)), b ?= a])),
    echoNL("false : OK"), nl.
